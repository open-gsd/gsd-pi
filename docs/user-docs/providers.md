...
| Cursor Agent | Local CLI subscription or API key | `CURSOR_API_KEY` | `cursor-agent` CLI required |
...
The setup wizard shows the Cursor Agent option only after GSD can run the CLI and detect authentication. Even when `CURSOR_API_KEY` is set, GSD still needs the local `cursor-agent` binary because requests are executed through that CLI.

**Environment variables:**

| Variable | Purpose |
|----------|---------|
| `CURSOR_API_KEY` | API key auth signal for the `cursor-agent` provider. Also hydrated from saved `cursor-agent` credentials in `~/.gsd/agent/auth.json`. |
...
### Cursor Agent not appearing in `/model`

**Cause:** GSD only registers the `cursor-agent` provider when the Cursor Agent CLI is available and authenticated. The provider is also hidden when `GSD_CURSOR_DISABLE=1`.

**Fix:** Confirm the CLI and auth state in the same shell where you launch GSD:
...
If the binary lives outside `PATH`, set `CURSOR_AGENT_BIN=/absolute/path/to/cursor-agent`. If your setup uses API-key auth, export `CURSOR_API_KEY` before launching GSD.
