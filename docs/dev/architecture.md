...
| **OpenClaw** | [`integrations/openclaw/`](../../integrations/openclaw/) | Native OpenClaw plugin (`open-gsd-openclaw`, npm `@opengsd/open-gsd-openclaw`) — ships `gsd-mcp-server` as a manifest MCP server so OpenClaw agents run GSD through its session tools, event-driven automatic project registration, native TaskFlow and optional Workboard synchronization, and factual heartbeat context. GSD retains execution and recovery; the plugin has no polling timer or process supervisor. See the package README. |
