...
**Critical synchronous execution:** Use only the GSD `subagent` tool for gate fanout. Do **not** use the native `Agent` tool or any dispatch with `run_in_background: true` — backgrounded subagents complete after your turn ends, so gate verdicts are never persisted and auto-mode wedges permanently. Each `subagent` call MUST set `run_in_background: false`, and your turn may NOT end until `gsd_save_gate_result` has persisted a verdict for EVERY gate in this unit's scope: {{gateIdList}}.
